...
- (void)about:(id)sender {
  UIAlertController *alert = [UIAlertController
      alertControllerWithTitle:@"TrollAppDuplicator"
                       message:@"by zznQ\n"
                               @"At: "
                               @"https://github.com/BreakOnCrash/"
                               @"TrollAppDuplicator\n"
...
- (void)tableView:(UITableView *)tableView
  ...
  UIAlertAction *cancel =
      [UIAlertAction actionWithTitle:@"Cancel"
                               style:UIAlertActionStyleCancel
                             handler:nil];
  UIAlertAction *duplicate = [UIAlertAction
      actionWithTitle:@"Yes"
                style:UIAlertActionStyleDefault
              handler:^(UIAlertAction *action) {
                UIAlertController *loadingAlert = [UIAlertController
                    alertControllerWithTitle:@"Duplicating"
                                     message:@"Please wait, this will take a "
                                             @"few seconds..."
                              preferredStyle:UIAlertControllerStyleAlert];
                [self presentViewController:loadingAlert
                                   animated:YES
                                 completion:nil];
                dispatch_async(
                    dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT,
                                              0),
                    ^{
                      NSString *ipa = [self.duplicator
                          duplicateAppWithBundleID:app[@"path"]
